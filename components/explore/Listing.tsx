        ...
        <div className={className}>
            <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl text-gray-500">
                    {isLoading
                        ? "Listing loading for given query"
                        : isError
                            ? JSON.stringify(isError)
                            : !listings || listings.length === 0
                                ? "No listings found for given query"
                                : `${listings.length} listing for given query`}
                </h3>
                <div
                    className={clsx("flex gap-1 invisible", {
                        visible: isLoading || (listings && listings.length),
                    })}
                >
                    <button
                        type="button"
                        className={clsx(
                            "cursor-pointer p-2 bg-gray-300 text-gray-700 rounded transition",
                            {
